...
struct ContentView: View {
    ...
    @State private var refreshId = UUID() // Forces view updates
    ...
    var body: some View {
        Group {
            if appData.currentUser != nil && (appData.currentRoomId != nil || appData.roomCode != nil) {
                ...
                .onChange(of: appData.treatmentTimerEnd) { newValue in handleTimerEndChange(newValue) }
                .onChange(of: appData.syncError) { newValue in showingSyncError = newValue != nil }
                .onChange(of: showingSetupWizard) { newValue in handleSetupWizardChange(newValue) }
                ...
                // Show authentication UI instead
    ...
    private func setupWizardSheet() -> some View {
        if showingCycleEndPopup {
            return AnyView(NewCycleSetupView(appData: appData, isNewCycleSetupActive: $isNewCycleSetupActive))
        } else {
        ...
        return VStack(alignment: .leading) {
            ...
            if let countdown = treatmentCountdown, countdown > 0 {
                VStack(spacing: 8) {
                    ...
                    ProgressView(value: 1.0 - (countdown / (appData.currentUser?.treatmentTimerDuration ?? 900)))
                        .progressViewStyle(LinearProgressViewStyle())
                        .tint(.purple)
                        .frame(height: 6)
                        .padding(.horizontal)
    ...
    func isItemCheckedToday(_ item: Item) -> Bool {
        guard let cycleId = appData.currentCycleId() else {
            print("isItemCheckedToday: No cycleId for item \(item.id)")
            return false
        }
        let today = Calendar.current.startOfDay(for: Date())
        let logs = appData.consumptionLog[cycleId]?[item.id] ?? []
        let isChecked = logs.contains { log in
            ...
            let matches = logDay == today
            print("Checking log \(log.date) (day: \(logDay)) against today \(today) = \(matches)")
            return matches
        }
        print("isItemCheckedToday: Item \(item.id) on \(today) = \(isChecked), logs: \(logs)")
        return isChecked
    }
    ...
    func toggleCheck(item: Item) {
        ...
        if isChecked {
            ...
            if item.category == .treatment && (appData.currentUser?.treatmentFoodTimerEnabled ?? false) {
                print("Unchecking treatment item \(item.id), stopping timer")
                stopTreatmentTimer()
            }
        } else {
            appData.logConsumption(itemId: item.id, cycleId: cycleId)
            print("Logged item \(item.id) for today")
            ...
            if item.category == .treatment && (appData.currentUser?.treatmentFoodTimerEnabled ?? false) {
                DispatchQueue.main.async {
                    ...
                    let isLastItem = unloggedTreatmentItems.isEmpty

                    print("!!! DEBUG: Treatment items total: \(treatmentItems.count), Checked: \(checkedTreatmentItems.count), Unlogged: \(unloggedTreatmentItems.count) !!!")
                    print("!!! DEBUG: Checked items: \(checkedTreatmentItems.map { $0.id }) !!!")
                    print("!!! DEBUG: Unlogged items: \(unloggedTreatmentItems.map { $0.id }) !!!")
                    ...
                    if !isLastItem {
                        ...
                        print("!!! DEBUG: Checking last treatment item \(item.id), STOPPING timer as no items remain !!!")
                        self.stopTreatmentTimer()
                    }
                }
            } else if item.category == .treatment {
                print("!!! DEBUG: Timer not enabled for user, skipping timer logic for item \(item.id) !!!")
            }
        }
    ...
    func resumeTreatmentTimer() {
        guard let endDate = appData.treatmentTimerEnd, endDate > Date() else {
            print("No valid timer to resume, endDate: \(String(describing: appData.treatmentTimerEnd))")
            stopTreatmentTimer()
            return
        }
    ...
    func updateTreatmentCountdown() {
        guard appData.currentUser?.treatmentFoodTimerEnabled ?? false else {
            if treatmentCountdown != nil {
                treatmentCountdown = nil
                print("Timer not enabled, clearing countdown")
            }
            return
        }
        if let endDate = appData.treatmentTimerEnd {
            ...
            treatmentCountdown = remaining
            print("Timer running, remaining: \(remaining), endDate: \(endDate)")

            let treatmentItems = currentItems().filter { $0.category == .treatment }
            let allChecked = treatmentItems.allSatisfy { isItemCheckedToday($0) }
            if allChecked {
                print("!!! DEBUG: All treatment items checked, stopping timer in updateTreatmentCountdown !!!")
                stopTreatmentTimer()
            } else if remaining <= 1 && !showingTimerAlert && !isCategoryComplete(.treatment) {
            ...
            treatmentCountdown = nil
            print("No timer endDate, timer inactive")
        }
    }
    ...
    func handleSetupWizardChange(_ newValue: Bool) {
    ...
    func onAppearActions() {
        ...
        if let endDate = appData.treatmentTimerEnd {
            if endDate > Date() {
                print("Resuming timer on appear, endDate: \(endDate)")
                resumeTreatmentTimer()
            } else {
                print("Timer expired on appear, clearing, endDate: \(endDate)")
                stopTreatmentTimer()
            }
        } else {
            print("No timer to resume on appear")
        }
    }
